...
namespace TeamCitySharp
{
    public class TeamCityClient : IClientConnection
    {
        ...
        {
            var build =
                _caller.Get<BuildConfig>(string.Format("/httpAuth/app/rest/buildTypes/name:{0}", buildConfigName));
        ...
        {
            var buildWrapper =
                _caller.Get<BuildTypeWrapper>(string.Format("/httpAuth/app/rest/projects/id:{0}/buildTypes", projectId));
        ...
        {
            var buildWrapper =
                _caller.Get<BuildTypeWrapper>(string.Format("/httpAuth/app/rest/projects/name:{0}/buildTypes",
                                                            projectName));
            if (buildWrapper == null) return new List<BuildConfig>();
            return buildWrapper.BuildType;
        }
        ...
        {
            var buildWrapper =
                _caller.Get<BuildWrapper>(string.Format("/httpAuth/app/rest/buildTypes/id:{0}/builds?status=SUCCESS",
                                                        buildConfigId));
        ...
        {
            List<Build> sb = SuccessfulBuildsByBuildConfigId(buildConfigId);
            if (sb == null) return null;

            return sb.FirstOrDefault();
        }

        public List<Build> FailedBuildsByBuildConfigId(string buildConfigId)
        {
            var buildWrapper =
                _caller.Get<BuildWrapper>(string.Format("/httpAuth/app/rest/buildTypes/id:{0}/builds?status=FAILURE",
                                                        buildConfigId));
        ...
        {
            List<Build> fb = FailedBuildsByBuildConfigId(buildConfigId);
            if (fb == null) return null;
            return fb.FirstOrDefault();
        }

        public Build LastBuildByBuildConfigId(string buildConfigId)
        {
            var buildWrapper =
                _caller.Get<BuildWrapper>(string.Format("/httpAuth/app/rest/buildTypes/id:{0}/builds", buildConfigId));
            if (buildWrapper.Build == null) return new Build();
        ...
        {
            var buildWrapper =
                _caller.Get<BuildWrapper>(string.Format("/httpAuth/app/rest/buildTypes/id:{0}/builds?status=ERROR",
                                                        buildConfigId));
        ...
        {
            List<Build> eb = ErrorBuildsByBuildConfigId(buildConfigId);
        ...
        {
            List<Build> builds = BuildsByUserName(userName);
